...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -f --from [arg]  from image. Default="localhost/ubuntu-nodeimage"
...
buildah run $buildcntr2 bash -c "mkdir /app"
...
buildah run $buildcntr2 -- bash -c "(pwd && ng build --output-path=/var/www/html)"
...
# buildah run $buildcntr2 -- bash -c "rm /etc/nginx/conf.d/default.conf"
buildah run $buildcntr2 -- bash -c "rm /etc/nginx/nginx.conf"

info "Configure nginx"
buildah copy $buildcntr2  nginx.conf /etc/nginx/nginx.conf

buildah run $buildcntr2 sh -c "chown -R nobody:nogroup  /var/www/html" 
buildah run $buildcntr2 sh -c "chown -R nobody:nogroup  /app" 
buildah run $buildcntr2 sh -c "chown -R nobody:nogroup  /var/lib/nginx" 
buildah run $buildcntr2 sh -c "chown -R nobody:nogroup  /var/log/nginx" 
buildah run $buildcntr2 sh -c "chown -R nobody:nogroup  /run" 

buildah config --cmd '/usr/bin/supervisord -c /etc/supervisor/conf.d/supervisord.conf' $buildcntr2 
info "Setup non root user"
buildah config --user nobody:nogroup $buildcntr2 
...
if [ -z "${arg_r}" ]
  then
    ...
    buildah commit --squash --rm --format docker $buildcntr2 node-baseimage:latest
  else 
  info "Commit image to ${arg_r}"
  buildah commit --squash --rm --format docker $buildcntr2 ${arg_r}node-baseimage:latest
  info "Pushing image to ${arg_r}"
  buildah push --tls-verify=false ${arg_r}node-baseimage:latest
fi
...
if [ -z "${arg_r}" ]
  then
    info "Registry url not defined"
    info "Usage : - podman run -p 8080:8080 -dt localhost:32000/node-baseimage -name node-baseimage"
  else 
  info "Usage : - podman run -p 8080:8080 -dt ${arg_r}node-baseimage -name node-baseimage"
fi
