...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -f --from [arg]  from image. Default="ubuntu-baseimage:latest"
...
buildah run $buildcntr2 bash -c  "apt-get update -y && apt-get install -y --no-install-recommends curl ca-certificates gnupg" 
...
if [ -z "$arg_r" ]
  then
    ...
    buildah commit --squash --rm --format docker $buildcntr2 ubuntu-nodeimage:latest
  else 
  info "Commit image to ${arg_r}ubuntu-nodeimage:latest"
  buildah commit --squash --rm --format docker $buildcntr2 ${arg_r}ubuntu-nodeimage:latest
  info "Pushing image to ${arg_r}ubuntu-nodeimage:latest"
  buildah push --tls-verify=false ${arg_r}ubuntu-nodeimage:latest
fi
...
if [ -z "$arg_r" ]
  ...
  info "Usage : - podman run -p 8080:8080 -dt ${arg_r}ubuntu-nodeimage:latest -name ubuntu-nodeimage"
fi